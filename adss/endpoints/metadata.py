...
class MetadataEndpoint:
    ...
    def get_schemas(self, **kwargs) -> List[str]:
        ...
        try:
            response = self.auth_manager.request(
                method="GET", url="/adss/v1/metadata/schemas", **kwargs
            )
            ...
            if hasattr(e, "response") and e.response.status_code == 401:
                raise AuthenticationError(
                    "Authentication required for protected schemas"
                )
            raise
            ...
            if hasattr(e, "response"):
                if e.response.status_code == 401:
                    ...
                    raise PermissionDeniedError(
                        f"Access denied to schema: {schema_name}"
                    )
        ...
        try:
            ...
            if hasattr(e, "response"):
                if e.response.status_code == 401:
                    ...
                    raise PermissionDeniedError(
                        f"Access denied to table: {schema_name}.{table_name}"
                    )
                elif e.response.status_code == 404:
                    raise ResourceNotFoundError(
                        f"Table not found: {schema_name}.{table_name}"
                    )
            raise
    ...
    def get_database_metadata(self, **kwargs) -> DatabaseMetadata:
        ...
        try:
            response = self.auth_manager.request(
                method="GET", url="/adss/v1/metadata/database", **kwargs
            )
            handle_response_errors(response)
