...
class QueriesEndpoint:
        ...
        **kwargs,
        ...
        if file:
            ...
            try:
                ...
                response = self.auth_manager.request(
                    method="POST", url="/adss/sync", data=data, files=files, **kwargs
                )
            finally:
                if close_file:
                    file_obj.close()
            ...
            # No file upload
            response = self.auth_manager.request(
                method="POST", url="/adss/sync", data=data, **kwargs
            )
        ...
        try:
            ...
            raise QueryExecutionError(
                f"Synchronous query execution failed: {str(e)}", query
            )
    ...
    def execute_async(
        ...
        **kwargs,
        ...
        try:
            ...
            raise QueryExecutionError(
                f"Failed to start asynchronous query: {str(e)}", query
            )
    ...
    def get_results(
        self, query_id: str, verbose: bool = False, **kwargs
    ) -> QueryResult:
        """
        ...
        if query.is_failed:
            raise QueryExecutionError(
                f"Cannot get results: Query failed with error: {query.error}", query_id
            )
        ...
        try:
            ...
            raise QueryExecutionError(
                f"Failed to get query results: {str(e)}", query_id
            )
    ...
    def execute_and_wait(
        ...
        **kwargs,
        ...
        completed_query = self.wait_for_completion(
            query_obj.id, timeout, poll_interval, **kwargs
        )

        if completed_query.is_failed:
            raise QueryExecutionError(
                f"Query failed with error: {completed_query.error}", query
            )
